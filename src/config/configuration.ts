...
const configuration = () => {
  ...
  const config = {
    ...
    oidc: {
      ...
      additionalAuthorizedParties: process.env.OIDC_ADDITIONAL_AUTHORIZED_PARTIES
        ? process.env.OIDC_ADDITIONAL_AUTHORIZED_PARTIES.split(",").map((v) => v.trim()).filter(Boolean)
